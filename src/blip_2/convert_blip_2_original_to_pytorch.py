...
from modeling_blip2 import Blip2ForQformerTraining
...
def create_rename_keys(config, model_name):
    ...
    if "qformer" in model_name:
        ...
        rename_keys.append(("Qformer.bert.embeddings.LayerNorm.weight", "text_model.qformer.embeddings.layernorm.weight"))
        rename_keys.append(("Qformer.bert.embeddings.LayerNorm.bias", "text_model.qformer.embeddings.layernorm.bias"))
        rename_keys.append(("Qformer.bert.embeddings.word_embeddings.weight", "text_model.qformer.embeddings.word_embeddings.weight"))
        rename_keys.append(("Qformer.bert.embeddings.position_embeddings.weight", "text_model.qformer.embeddings.position_embeddings.weight"))
...
def get_blip2_config(model_name, eos_token_id):
    ...
    elif "itm" in model_name or "qformer" in model_name:
        text_config = {}
    else:
        raise ValueError("Model name not supported")

    if "itm" in model_name or "qformer" in model_name:
        config = Blip2Config(
            vision_config=vision_config,
            qformer_config=Blip2QFormerConfig(vocab_size=30522 + 1, use_qformer_text_input=True).to_dict(),  # +1 for [DEC]
        )
    else:
...
def convert_blip2_checkpoint(
    ...
    if "itm" in model_name:
        ...
        hf_model = Blip2ForQformerTraining(config).eval()
    else:
    ...
    model_name_to_original = {
        ...
        "blip2-qformer": ("blip2", "pretrain"),
        "blip2-qformer-coco": ("blip2", "coco"),
    }
    ...
    for key, val in state_dict.copy().items():
        ...
        if "qformer" in model_name:
            if key.startswith("Qformer.bert"):
                key = key.replace("Qformer.bert", "text_model.qformer")
            if key.startswith("Qformer.cls"):
                key = key.replace("Qformer.cls", "text_model.cls")
        else:
            if key.startswith("Qformer.bert"):
    ...
    if "itm" in model_name:
        ...
        assert unexpected_keys == ["temp", "text_model.qformer.embeddings.position_ids"]
    else:
    ...
    if "itm" in model_name:
        ...
        with torch.no_grad():
            original_image_embeds = original_model.extract_features(
            ...
            print("Image features comparison passed")

            original_text_embeds = original_model.extract_features(
                {"text_input": [caption]}, mode="text"
            ).text_embeds_proj
            text_embeds = hf_model.get_text_features(
                input_ids=text_tokens.input_ids,
                attention_mask=text_tokens.attention_mask,
            ).text_embeds
            assert torch.allclose(original_text_embeds.to(target_dtype), text_embeds, atol=1e-4)
            print("Text features comparison passed")
            
            original_multimodal_embeds = original_model.extract_features(
            ...
            assert torch.allclose(original_multimodal_embeds.to(target_dtype), multimodal_embeds, atol=1e-4)
            print("Multimodal features comparison passed")
            ...
            original_outputs = original_model.generate(
                {"image": original_pixel_values}, use_nucleus_sampling=True, **generation_config
            )
            ...
            print("HF generation:", tokenizer.batch_decode(outputs, skip_special_tokens=True))
    else:
